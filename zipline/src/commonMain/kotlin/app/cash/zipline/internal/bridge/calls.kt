...
internal val failureResultSerializer = CallResultSerializer(Int.serializer())

internal class InternalCallSerializer(
  private val endpoint: Endpoint,
) : KSerializer<InternalCall> {

  override val descriptor: SerialDescriptor = buildClassSerialDescriptor("InternalCall") {
  ...
  override fun deserialize(decoder: Decoder): InternalCall {
    return decoder.decodeStructure(descriptor) {
      ...
      return@decodeStructure InboundCall2(
        serviceName,
        service,
        functionName,
        function,
...
internal class CallResultSerializer<T>(
  ...
  override val descriptor: SerialDescriptor = buildClassSerialDescriptor("Result") {
    element("success", resultSerializer.descriptor)
  ...
  override fun serialize(encoder: Encoder, value: Result<T>) {
    encoder.encodeStructure(descriptor) {
      if (value.isSuccess) {
        encodeSerializableElement(descriptor, 0, resultSerializer, value.getOrNull()!!)
      } else {
    ...
    return decoder.decodeStructure(descriptor) {
      ...
      while (true) {
        result = when (val index = decodeElementIndex(descriptor)) {
          0 -> Result.success(decodeSerializableElement(descriptor, 0, resultSerializer))
